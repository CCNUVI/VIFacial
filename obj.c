...
struct obj_surf
{
    ...
    GLuint pibo;		// polygon index buffer object
    GLuint libo;		// line index buffer object
...
struct obj_file
{
    ...
    struct obj_vert *vv;	// array of vert
...
static void *read_raw(const char *filename, int *w, int *h, int *b)
{
	unsigned char *p = NULL;
	...
	*b = 3;
	...
	if ((fin = fopen(filename,  "rb")))
	{
		...
		{
			register int y;
			for (y = 0; y < total; y+=1) /// rgb texture this time ...
				p[y]	= (char) fgetc(fin);
...
void *obj_read_image(const char *filename, int *w, int *h, int *b)
{
    ...
    //    return read_raw(filename, w, h, b);
...
void obj_draw_surf(int fi, int si)
{
    ...
    if (0 < sp->pc || sp->lc > 0)
    {
		...
		// James: do not use this material property for morpher...
